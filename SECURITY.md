# Security Policy

If you find a security issue, please do not post it publicly first.

## Report A Vulnerability

Send the details to the maintainer directly with:

- a short summary
- affected route or file
- reproduction steps
- impact
- any suggested fix

If email is configured for manual billing support, use that address first. Otherwise open a private GitHub security advisory if available.

## Scope

The highest-priority reports are:

- auth or session bypass
- payment or plan escalation bugs
- data access across users
- prompt injection leading to unsafe file or data handling
- API abuse paths that can burn credits or cause denial of service

## Expectations

- good-faith testing only
- no mass scanning or destructive activity
- no access to user data beyond what is needed to demonstrate the issue

ApplyX is in beta, so reports that improve real user safety and cost control are especially valuable.
